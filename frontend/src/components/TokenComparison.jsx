...
export default function TokenComparison({ sentiments }) {
  ...
  // Find best and worst with safety check
  const best = chartData.length > 0 ? chartData.reduce((prev, current) =>
    prev.score > current.score ? prev : current
  ) : { token: 'N/A', score: 0 };
  
  const worst = chartData.length > 0 ? chartData.reduce((prev, current) =>
    prev.score < current.score ? prev : current
  ) : { token: 'N/A', score: 0 };
